...
const props = defineProps<{ form: any; t: any }>()
...
<template>
  <section class="bg-white p-5 rounded-xl border shadow-sm">
    ...
    <div v-if="form.bannerUrl" class="relative">
      ...
      <button type="button" @click="form.bannerUrl = ''" class="absolute top-2 right-2 bg-red-600 text-white px-3 py-1 rounded-md text-xs shadow hover:bg-red-700">{{ t.removeImg }}</button>
    </div>
    <div v-else class="flex flex-col items-center justify-center h-32 border-2 border-dashed border-gray-300 rounded-lg hover:bg-gray-50 transition relative cursor-pointer">
