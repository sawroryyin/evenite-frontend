    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
      <div class="bg-gradient-to-r from-purple-800 to-indigo-900 p-5 text-white flex justify-between items-center">
        <div class="flex items-center gap-3">
          <div class="w-12 h-12 rounded-full bg-white/10 flex items-center justify-center border-2 border-white/30 text-white font-bold text-[16px] shadow-inner">
        ...
        <button 
          v-if="!isEditing" 
          @click="enableEdit" 
          class="bg-white/15 hover:bg-white/25 text-white border border-white/20 py-1.5 px-3 rounded-lg font-bold transition text-[11px] flex items-center gap-1 cursor-pointer">
      ...
      <form @submit.prevent="saveProfile" class="p-5 space-y-4 text-[12px]">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
          <div>
            ...
            <input v-if="isEditing" type="text" v-model="profile.firstName" class="w-full border border-gray-200 rounded-lg py-2 px-2.5 focus:ring-1.5 focus:ring-purple-400 focus:border-purple-400 outline-none font-bold bg-[#fafafa] text-[12px]" required />
          ...
          <div>
            ...
            <input v-if="isEditing" type="text" v-model="profile.lastName" class="w-full border border-gray-200 rounded-lg py-2 px-2.5 focus:ring-1.5 focus:ring-purple-400 focus:border-purple-400 outline-none font-bold bg-[#fafafa] text-[12px]" required />
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
          <div>
            ...
            <input v-if="isEditing" type="email" v-model="profile.email" disabled class="w-full border border-gray-100 bg-gray-50 text-gray-400 rounded-lg py-2 px-2.5 cursor-not-allowed font-medium text-[12px]" />
          ...
          <div>
            ...
            <input v-if="isEditing" type="text" v-model="profile.studentId" class="w-full border border-gray-200 rounded-lg py-2 px-2.5 focus:ring-1.5 focus:ring-purple-400 focus:border-purple-400 outline-none font-bold bg-[#fafafa] text-[12px]" />
        ...
        <div v-if="isEditing" class="pt-2 flex justify-end gap-2">
          <button @click="cancelEdit" type="button" class="px-4 py-1.5 border border-gray-200 text-gray-600 hover:bg-gray-50 font-bold rounded-lg transition-colors cursor-pointer text-[11px]">
            Cancel
          </button>
          <button type="submit" class="bg-purple-600 hover:bg-purple-700 text-white font-bold py-1.5 px-4 rounded-lg shadow-sm transition-colors cursor-pointer text-[11px]">
            Save Changes
          </button>
        </div>
      </form>
