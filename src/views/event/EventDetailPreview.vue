...
<template>
  <div class="font-['Lato'] max-w-6xl mx-auto space-y-6 pb-6">
    
    <div class="bg-white rounded-2xl shadow-sm overflow-hidden border border-[#131B2B]/10">
      <div class="w-full h-48 md:h-72 relative border-b border-[#131B2B]/10">
        <img v-if="event.bannerUrl" :src="event.bannerUrl" class="w-full h-full object-cover" />
        <div v-else class="w-full h-full flex items-center justify-center text-[#131B2B]/70 font-bold text-sm bg-[#131B2B]/5">
          No Banner Uploaded
        </div>
      </div>
      ...
      <div class="p-6 md:p-8 grid grid-cols-1 md:grid-cols-3 gap-8">
        <div class="md:col-span-2 space-y-8">
          <div>
            <h1 class="text-2xl md:text-3xl font-['Nunito'] font-black text-[#131B2B] mb-3 leading-tight tracking-tight">
              {{ viewLang === 'en' ? event.title.en : event.title.th || 'Untitled Event' }}
            </h1>
            <p class="text-[#131B2B]/80 whitespace-pre-wrap text-sm md:text-base leading-relaxed">
            ...
            <div v-if="event.category?.length > 0" class="mt-6 flex flex-wrap gap-2">
              <span v-for="cat in event.category" :key="cat" class="bg-[#131B2B]/5 text-[#131B2B] px-2.5 py-1 rounded-lg text-[10px] font-black uppercase tracking-widest shadow-sm border border-[#131B2B]/10">
                {{ cat }}
              </span>
            </div>
          </div>
          ...
          <div v-if="event.agenda?.length > 0">
            <h2 class="text-lg font-['Nunito'] font-black text-[#131B2B] border-b border-[#131B2B]/10 pb-2 mb-5">Event Agenda</h2>
            <ul class="space-y-5 relative border-l-2 border-[#131B2B]/10 ml-2.5 pl-6">
              <li v-for="(item, index) in event.agenda" :key="index" class="relative">
                <span class="absolute -left-7.75 top-1 w-3.5 h-3.5 bg-[#131B2B] rounded-full border-2 border-white shadow-sm"></span>
                <p class="text-sm font-black text-[#131B2B] tracking-wide">{{ item.time || '--:--' }}</p>
                <p class="text-[#131B2B] font-semibold text-sm mt-0.5">{{ viewLang === 'en' ? item.activity.en : item.activity.th }}</p>
              </li>
            </ul>
          </div>
        </div>
        ...
        <div class="space-y-5">
          <div class="bg-[#131B2B]/5 p-5 rounded-2xl border border-[#131B2B]/10 space-y-5">
            <div class="flex items-start gap-3">
              <svg class="w-5 h-5 text-[#131B2B] mt-0.5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
              </svg>
              <div>
                <h3 class="text-xs font-black text-[#131B2B]/70 uppercase tracking-widest mb-1.5">Date & Time</h3>
                <template v-if="isSingleDay">
                  <p class="text-sm font-bold text-[#131B2B]">{{ formatDateObj(event.startAt).date }}</p>
                  <p class="text-xs text-[#131B2B]/70 font-semibold mt-0.5">
                    {{ formatDateObj(event.startAt).time }}
                    <template v-if="event.endAt"> - {{ formatDateObj(event.endAt).time }}</template>
                  </p>
                </template>
                <template v-else>
                  <p class="text-xs font-semibold text-[#131B2B]">
                    <span class="text-[#131B2B] font-bold">Start - </span>{{ formatDateObj(event.startAt).full }}
                  </p>
                  <p v-if="event.endAt" class="text-xs font-semibold text-[#131B2B] mt-1.5">
                    <span class="text-[#131B2B] font-bold">End &nbsp;&nbsp;- </span>{{ formatDateObj(event.endAt).full }}
                  </p>
                </template>
              </div>
            </div>
            
            <hr class="border-[#131B2B]/10" />
            ...
            <div class="space-y-4">
              <div class="flex items-start gap-3">
                ...
                <div>
                  <h3 class="text-xs font-black text-[#131B2B]/70 uppercase tracking-widest mb-1">Location</h3>
                  <p v-if="event.isOnline" class="text-xs font-black text-[#131B2B] bg-[#131B2B]/10 inline-block px-2.5 py-1 rounded-lg mt-1">Online Event</p>
                  <p v-else class="text-sm font-semibold text-[#131B2B]">
                    {{ viewLang === 'en' ? event.location.en : event.location.th || 'TBA' }}
                  </p>
                </div>
              </div>
              
              <div v-if="!event.isOnline" class="w-full space-y-2 pt-1">
                <div v-if="mapEmbedUrl" class="w-full rounded-xl overflow-hidden border border-[#131B2B]/10 shadow-sm bg-[#131B2B]/5 h-32 md:h-40">
                  <iframe width="100%" height="100%" frameborder="0" style="border:0;" :src="mapEmbedUrl" allowfullscreen="false" loading="lazy"></iframe>
                </div>
                <a v-if="event.mapLink" :href="event.mapLink" target="_blank" class="text-[#131B2B] hover:text-[#131B2B]/70 hover:underline text-xs font-bold inline-block mt-1 transition-colors">
                  Open in Google Maps ↗
                </a>
              </div>
            </div>
          ...
          <div v-if="event.seatLimit || event.hasCatering || event.contactName || event.contactEmail" class="bg-[#131B2B]/5 p-5 rounded-2xl border border-[#131B2B]/10 space-y-5">
            <div v-if="event.seatLimit" class="flex items-start gap-3">
              <svg class="w-5 h-5 text-[#131B2B] mt-0.5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z" />
              </svg>
              <div>
                <h3 class="text-xs font-black text-[#131B2B]/70 uppercase tracking-widest mb-1">Capacity</h3>
                <p class="text-sm font-bold text-[#131B2B]">{{ event.seatsTaken }} / {{ event.seatLimit }} seats booked</p>
              </div>
            </div>
          
            <hr v-if="event.seatLimit && (event.hasCatering || event.contactName)" class="border-[#131B2B]/10" />
            ...
            <div v-if="event.hasCatering" class="flex items-start gap-3">
              ...
              <div>
                <h3 class="text-xs font-black text-[#131B2B]/70 uppercase tracking-widest mb-1">Catering</h3>
                <p class="text-sm font-bold text-[#131B2B]">{{ event.isCateringFree ? 'Free Catering' : 'Paid Catering' }}</p>
                <p class="text-xs text-[#131B2B]/70 font-semibold mt-1 leading-relaxed">
                  {{ viewLang === 'en' ? event.cateringDescription?.en : event.cateringDescription?.th }}
                </p>
              </div>
            </div>

            <hr v-if="event.hasCatering && (event.contactName || event.contactEmail)" class="border-[#131B2B]/10" />
            ...
            <div v-if="event.contactName || event.contactEmail" class="flex items-start gap-3">
              ...
              <div>
                <h3 class="text-xs font-black text-[#131B2B]/70 uppercase tracking-widest mb-1">Organizer Contact</h3>
                <p class="text-sm font-bold text-[#131B2B]">{{ event.contactName }}</p>
                <p class="text-sm mt-0.5">
                  <a :href="'mailto:'+event.contactEmail" class="text-[#131B2B] font-semibold hover:underline hover:text-[#131B2B]/70 transition-colors">{{ event.contactEmail }}</a>
                </p>
                <p v-if="event.contactPhone" class="text-xs font-semibold text-[#131B2B]/70 mt-1">{{ event.contactPhone }}</p>
              </div>
            </div>
          </div>

          <div v-if="(hasRegistration || hasFeedback || userTicket) && !isOrganizer && !isLoadingTicket" class="bg-[#131B2B]/5 p-6 rounded-2xl border border-[#131B2B]/10 shadow-sm space-y-4">
            
            <div class="text-center">
              <h3 class="text-lg font-['Nunito'] font-black text-[#131B2B] tracking-tight">
                {{ userTicket?.registration?.status === 'CONFIRMED' ? 'You are registered!' : 'Join the Experience' }}
              </h3>
              <p class="text-xs font-semibold text-[#131B2B]/70 mt-1">
            ...
            <div v-if="!userTicket || userTicket?.registration?.status === 'CANCELLED'" class="flex flex-col gap-3 mt-4">
              <p v-if="isEventOver || isEventFull || isEventStarted" class="text-center text-[#131B2B]/80 text-sm font-bold py-2">
              ...
              <button v-if="hasRegistration && !isEventFull && !isEventStarted && !isEventOver" @click="navigateToSubmitForm('REGISTRATION')" 
                class="w-full bg-[#131B2B] hover:bg-[#131B2B]/80 text-white shadow-sm transform active:scale-95 transition-all py-3.5 px-4 rounded-xl font-bold text-sm flex items-center justify-center gap-2">
            ...
            <div v-if="userTicket?.registration?.status === 'CONFIRMED'" class="flex flex-col gap-3 mt-4">
              <p v-if="isEventOver || isEventStarted" class="text-center text-[#131B2B]/80 text-sm font-bold py-2 px-1">
                {{ isEventOver ? 'This event is already over.' : 'This event has already started. Cancellation is no longer available.' }}
              </p>

              <div class="flex gap-2 w-full">
                <button @click="viewTicket" class="flex-1 bg-[#131B2B] hover:bg-[#131B2B]/80 text-white shadow-sm transform active:scale-95 transition-all py-3.5 px-4 rounded-xl font-bold text-sm flex items-center justify-center gap-2">
                  View ticket
                </button>
                ...
                <button 
                  ...
                  class="w-14 bg-white hover:bg-[#131B2B]/5 text-[#131B2B] hover:text-[#131B2B]/80 border border-[#131B2B]/10 shadow-sm transform active:scale-95 transition-all rounded-xl flex items-center justify-center shrink-0 cursor-pointer"
                >
              ...
              <button v-if="hasFeedback && isEventOver" @click="navigateToSubmitForm('FEEDBACK')" class="w-full bg-white hover:bg-[#131B2B]/5 text-[#131B2B] border border-[#131B2B]/10 shadow-sm transform active:scale-95 transition-all py-3.5 px-4 rounded-xl font-bold text-sm flex items-center justify-center gap-2">
                <svg class="w-5 h-5 text-[#131B2B]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <button v-if="!isEventStarted && !isEventOver" @click="confirmCancelRegistration" 
                class="w-full bg-red-50 hover:bg-red-100 text-red-600 border-red-200 border shadow-sm transform active:scale-95 transition-all py-3.5 px-4 rounded-xl font-bold text-sm flex items-center justify-center gap-2">
                Cancel Registration
              </button>
            </div>
          </div>

          <div v-if="isOrganizer" class="bg-white p-5 rounded-2xl border border-[#131B2B]/10 shadow-sm space-y-4">
            <h3 class="text-base font-['Nunito'] font-black text-[#131B2B] mb-2">Event Forms</h3>
            <div class="flex flex-col gap-3">
              <button @click="router.push(`/events/${event.id || 'new'}/forms/REGISTRATION?source=preview`)" class="w-full bg-white hover:bg-[#131B2B]/5 text-[#131B2B] hover:text-[#131B2B]/80 border border-[#131B2B]/10 py-3.5 rounded-xl font-bold text-sm shadow-sm transition-all flex items-center justify-center gap-2">
              ...
              <button @click="router.push(`/events/${event.id || 'new'}/forms/FEEDBACK?source=preview`)" class="w-full bg-white hover:bg-[#131B2B]/5 text-[#131B2B] hover:text-[#131B2B]/80 border border-[#131B2B]/10 py-3.5 rounded-xl font-bold text-sm shadow-sm transition-all flex items-center justify-center gap-2">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path></svg>
