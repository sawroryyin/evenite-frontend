...
<template>
  <div v-if="message.isDivider" :id="message.id" class="flex w-full items-center justify-center my-4 py-1">
    ...
    <span class="px-3 text-xs font-bold text-[#534AB7] tracking-widest uppercase bg-[#F4F4FA]">
      {{ message.content }}
    </span>
    <div class="h-px bg-[#534AB7]/30 flex-1"></div>
  </div>
  ...
  <div v-else class="flex w-full items-end" :class="isMine ? 'justify-end' : 'justify-start'">
    ...
    <div class="flex flex-col max-w-[78%]" :class="isMine ? 'items-end' : 'items-start'">
      ...
      >
        <span v-if="!isMine" class="text-xs font-bold text-[#26215C]/80 tracking-wide">
          {{ message.sender.name }}
          <span v-if="message.sender.role === 'ORGANIZER'" class="text-[#534AB7] ml-0.5">(Organizer)</span>
        </span>
        <span class="text-xs font-semibold text-[#26215C]/40">{{ formatTime(message.createdAt) }}</span>
      </div>
      ...
      >
        <div v-if="message.isAnnouncement" class="text-xs font-bold uppercase tracking-widest text-[#CECBF6] mb-1 flex items-center">
